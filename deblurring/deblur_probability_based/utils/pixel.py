...
def find_cores_from_classification(region, shared):
    ...
    for coord in region:
        if confidence[coord][2] > max_confidence:
            ret = coord
            max_confidence = confidence[coord][2]
    ...
    for dx, dy in [(1, 0), (-1, 0), (0, -1), (0, 1)]:
        ...
        if 0 <= cx < l and 0 <= cy < l:
            ...
            for c_i in pixel_map[cx][cy].color_core:
                color_dist = color_metric(c_i, anti_32[cx, cy])
